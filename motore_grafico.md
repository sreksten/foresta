...
Le immagini dei personaggi e delle locazioni vengono da `ClassePersonaggioImmagine`/`ImageCache`; quelle di tipo risorsa sono caricate al primo uso con `BufferedImageBuilder.provaACaricare()` (che, a differenza di `buildBufferedImage()`, non chiude il gioco se la risorsa manca) e dimenticate a fine intermezzo (`svuota()`, chiamato da `DisplayableCanvas.iniziaGioco()`). `disegnaAl(graphics, secondi)` permette di disegnare la pagina a un istante preciso, utile per verificarla fuori dal gioco.
