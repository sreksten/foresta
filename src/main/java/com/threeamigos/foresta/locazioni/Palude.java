...
public class Palude extends LocazioneBase {
	...
	public void descrivi(GruppoGiocatore g, GruppoAvversario gng) {
		BusEventi.pubblica(new EventoMessaggio("Qui, nel mezzo di una malsana e pericolosa palude, " + g.chi() + " non trova nulla."));
	}

	@Override
	public Stato impostaAzioni(GruppoGiocatore g, GruppoAvversario gng, Comando azione) {
		if (isLocazioneVisitata() || Dado.tira(10) > 3) {
			BusEventi.pubblica(new EventoMessaggio("Il posto però non promette nulla di buono e " + g.chi() + " decide di non restare."));
			setCompleta(true);
			return Stato.FINE_LOCAZIONE;
		}
		BusEventi.pubblica(new EventoMessaggio("Approfittando della quiete del posto, " + g.chi() + " riposerà un poco."));
