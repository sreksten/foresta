...
`processaComando(Comando)` (righe 219-234) non è il singolo dispatcher che invoca un handler e basta: è un **ciclo** che continua a chiamare `eseguiPasso(...)` finché lo stato risultante non decide di fermarsi e aspettare un input reale. `eseguiPasso` (righe 236-252) sceglie quale delle due mappe consultare in base a se il comando corrente è `null` (ingresso automatico) o reale, e ogni handler:
...
- `NotizieMD` (`motore/modellodati/NotizieMD.java`) salva due liste: gli ultimi messaggi mostrati nel pannello di testo (uno per riga, preceduti dal conteggio) e le ultime `Notizia` (`id|corpo`). Due fragilità del formato: un messaggio contenente un vero a-capo spezzerebbe la lettura (oggi non succede: nei testi gli a-capo sono scritti come `\n` letterale), e il corpo di una notizia viene riletto con `StringTokenizer` su `|`, quindi un `|` nel testo lo troncherebbe. Dopo un caricamento riuscito `Automa` pubblica `InternoCaricamentoCompletato` con i messaggi salvati, e la UI ripopola il pannello di testo; resta aperto (FIXME in `Automa`) che frasi e paragrafi vengono ripristinati tutti come messaggi separati, perdendo la distinzione.
