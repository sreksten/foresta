...
- **Avvio**: `LOGO_INIZIALE`, il primissimo stato, solo all'avvio: la UI traccia il logo 3AM mentre si caricano le risorse; il motore precarica grammatiche e generatore di artefatti (in un thread a parte, o sullo stesso thread nei test) e passa a `INTRO` quando riceve sia `InternoFineLogoIniziale` sia `InternoPrecaricamentoMotoreCompletato`. I comandi in questo stato si ignorano; non ci si torna più.
...
4. Il risultato finale è un **valore percentuale 0-100** confrontato con un tiro di `Dado.tira(100)` (vedi sotto) per determinare colpito/mancato.
...
`Dado` (`motore/Dado.java`) è l'unica sorgente di caso del motore: un `Random` statico (`Dado.sorgente()`) da cui passano i lanci e le probabilità (`Dado.probabilita()`, un double in [0, 1)). Per i test:

- `Dado.impostaSeme(long)` rende ripetibile una partita intera. `GeneratoreArtefattiTabelle.ISTANZA` e le grammatiche (`GrammarBean`, compresa quella degli artefatti, che ne decide nome, modificatori e quindi prezzo) usano direttamente la sorgente: seguono il seme ma non consumano i lanci truccati.
- `Dado.trucca(Number...)` mette in coda i prossimi risultati (`Integer` per i lanci, `Double` per le probabilità); finita la coda si torna al caso. Un trucco fuori intervallo o del tipo sbagliato è un errore, e i lanci senza scelta (una faccia, min = max) non consumano trucchi. `Dado.ripristina()` svuota la coda, `Dado.trucchiRimasti()` dice se il test ha lanciato quanto previsto. Esempi in `DadoTest` e `CombattimentoTruccatoTest`.
...
- **Test di scenario sull'`Automa`**: `PartitaDiTest` (tra i test, pacchetto `motore`) guida una partita al posto del giocatore e della UI. `PartitaDiTest.nuova(seme)` azzera il bus (`BusEventi.azzera()`), ne rende immediata la consegna sul thread del test (`BusEventi.impostaConsegna(Runnable::run)`: nel gioco tutto gira sull'EDT, quindi la consegna è già sincrona), fissa il seme del `Dado`, ricrea `ModelloDati` e i singleton dei gruppi (`azzeraIstanza()`), e usa salvataggi e classifica in memoria e un `TemporizzatoreManuale` che scatta solo con `scatta()`. I comandi e i testi passano dal bus come dalla UI; un `InternoErrore` o `InternoException` fa fallire il test; gli intermezzi vengono fatti scorrere da soli dopo ogni passo (`nonSaltareIntermezzi()` per provarli). `iniziaCon(nome, sesso, classe, primaDiIniziare)` crea il protagonista e lascia preparare il mondo tra il nome e il sesso, per esempio con `spostaGruppoIn(ClassiLocazione.CITTA_NYENA)`. Esempi in `ScenarioArmaioloTest` e `ScenarioPartitaTest` (stesso seme, stessa Foresta e stessi magazzini; salvataggio e rilettura). Un'avvertenza: in `Automa.inizializzaGioco` e `GruppoGiocatore.reimposta` ci sono ancora le dotazioni "PER TEST" (monete, pozioni, incantesimi, artefatti) di cui gli scenari tengono conto.
