...
- `Dado.impostaSeme(long)` rende ripetibili i lanci. `GeneratoreArtefattiTabelle.ISTANZA` usa direttamente la sorgente: segue il seme ma non consuma i lanci truccati. Le grammatiche (`GrammarBean`) hanno invece un loro `Random` e restano fuori dal seme: i testi cambiano, le decisioni di gioco no.
